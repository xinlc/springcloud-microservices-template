package com.example.common.async;

import com.example.common.config.ThreadPoolConfig;
import com.example.common.utils.SpringUtil;
import com.example.common.utils.ThreadsUtil;

import java.util.TimerTask;
import java.util.concurrent.ScheduledExecutorService;
import java.util.concurrent.TimeUnit;

/**
 * 异步任务管理器
 *
 * @author Leo
 * @date 2020.04.12
 */
public class AsyncManager {
	/**
	 * 操作延迟10毫秒
	 */
	private final int OPERATE_DELAY_TIME = 10;

	/**
	 * 异步操作任务调度线程池
	 */
	private ScheduledExecutorService executor = SpringUtil.getBean(ThreadPoolConfig.SCHEDULED_EXECUTOR_SERVICE_NAME);

	/**
	 * 单例模式
	 */
	private AsyncManager() {
	}

	private static AsyncManager me;

	static {
		me = new AsyncManager();
	}

	public static AsyncManager me() {
		return me;
	}

	/**
	 * 执行任务
	 *
	 * @param task 任务
	 */
	public void execute(TimerTask task) {
		executor.schedule(task, OPERATE_DELAY_TIME, TimeUnit.MILLISECONDS);
	}

	/**
	 * 停止任务线程池
	 */
	public void shutdown() {
		ThreadsUtil.shutdownAndAwaitTermination(executor);
	}
}
