...
# docker 镜像仓库地址
docker_registry_uri="registry-vpc.cn-beijing.aliyuncs.com/example"
docker_file_path="${work_path}/doc/docker/Dockerfile"

init() {

	# 截取字符 10:example-abc -> 10
...
if test "$#" -ne 3; then
  # echo '请传入要构建的项目id'
  echo -e "$red 请输入正确参数：<Project Id> <development | testing | production> <Build Version>$none"
  show_projects
  exit 1
fi
